...
function evidenceFromAsset(asset: ImagePicker.ImagePickerAsset): IdentityEvidenceFile {
...
export default function IdentityVerificationScreen() {
  ...
  const load = useCallback(async () => {
    if (!session) {
      router.replace({ pathname: "/login", params: { returnTo: "/identity-verification" } });
      return;
    }
    setLoading(true);
    setError(null);
  ...
  const submit = async () => {
    ...
    if (!user.eligibility.isEligible) {
      setError("Confirme sua data de nascimento e a elegibilidade 18+ antes de enviar documentos.");
      return;
    }
    if (!front || !selfie) {
      setError("Selecione a frente do documento e a selfie segurando o documento.");
      return;
    }
    ...
    try {
      ...
      if (status === 409) {
        ...
        setError("Sua conta ainda não está elegível para enviar a verificação.");
      } else {
        setError("Não foi possível enviar as evidências. Confira as imagens e tente novamente.");
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const submissionAllowed = user.eligibility.isEligible && canSubmit(currentStatus);

  return (
    <ScrollView contentContainerStyle={styles.content}>
      <View style={styles.card}>
        <Text accessibilityRole="header" style={styles.title}>Verificação de identidade</Text>
        <Text style={styles.muted}>{statusCopy(currentStatus)}</Text>
        <Text style={styles.helper}>
          Suas evidências são privadas e usadas apenas para verificação. O app não mostra endereços de armazenamento, hashes ou links permanentes desses arquivos.
        </Text>
      </View>
      ...
      {!user.eligibility.isEligible ? (
        <View style={styles.warning}>
          ...
          <Text style={styles.muted}>Atualize sua data de nascimento em Minha conta antes de continuar.</Text>
          <AppButton label="Abrir Minha conta" onPress={() => router.push("/account")} variant="secondary" />
        </View>
      ) : null}
          ...
          <View style={styles.options}>
            {documentTypes.map((option) => (
              <Pressable
                ...
                style={[styles.option, documentType === option.value && styles.optionSelected]}
              >
                <Text style={[styles.optionText, documentType === option.value && styles.optionTextSelected]}>{option.label}</Text>
              </Pressable>
            ))}
          </View>

          <Text style={styles.sectionTitle}>2. Selecione as evidências</Text>
          <EvidenceRow label="Frente do documento · obrigatória" selected={Boolean(front)} onPress={() => void pick("front")} />
          <EvidenceRow label="Verso do documento · opcional" selected={Boolean(back)} onPress={() => void pick("back")} />
          <EvidenceRow label="Selfie segurando o documento · obrigatória" selected={Boolean(selfie)} onPress={() => void pick("selfie")} />

          <Text style={styles.helper}>As imagens selecionadas ficam apenas em memória nesta tela e são removidas do estado após o envio ou ao sair.</Text>
          {error ? <Text accessibilityLiveRegion="polite" style={styles.error}>{error}</Text> : null}
          {submitted ? <Text accessibilityLiveRegion="polite" style={styles.success}>Evidências enviadas para análise.</Text> : null}
          <AppButton disabled={submitting} label={submitting ? "Enviando..." : "Enviar para verificação"} onPress={() => void submit()} />
        </View>
      ) : null}
...
function EvidenceRow({ label, selected, onPress }: { label: string; selected: boolean; onPress: () => void }) {
  return (
    <View style={styles.evidenceRow}>
      <View style={styles.evidenceText}>
        <Text style={styles.evidenceLabel}>{label}</Text>
        <Text style={styles.muted}>{selected ? "Imagem selecionada" : "Nenhuma imagem selecionada"}</Text>
      </View>
      <AppButton label={selected ? "Trocar" : "Selecionar"} onPress={onPress} variant="secondary" />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: colors.surface, borderRadius: radius.lg, gap: spacing.md, padding: spacing.lg },
  warning: { backgroundColor: colors.surface, borderRadius: radius.lg, gap: spacing.md, padding: spacing.lg },
  ...
  option: { borderColor: colors.border, borderRadius: radius.full, borderWidth: 1, paddingHorizontal: spacing.md, paddingVertical: spacing.sm },
