...
const reasons: Array<{ value: ListingCloseReason; label: string }> = [
  ...
  { value: "PROPERTY_UNAVAILABLE", label: "A moradia não está mais disponível" },
  ...
  return (
    <ScrollView contentContainerStyle={styles.content}>
      <View style={styles.card}>
        <Text accessibilityRole="header" style={styles.title}>Encerrar anúncio</Text>
        <Text style={styles.muted}>
          O motivo estruturado ajuda a medir resultados da Beta sem transformar o encerramento em uma penalidade.
        </Text>
      </View>
      <View style={styles.card}>
        <Text style={styles.sectionTitle}>Por que este anúncio será encerrado?</Text>
        {reasons.map((item) => (
          <AppButton
            key={item.value}
            label={item.label}
